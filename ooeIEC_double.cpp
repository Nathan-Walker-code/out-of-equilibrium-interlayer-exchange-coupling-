...
double voltage=-0.01;                                    /*bias - VARIABLE*/
...
const int left_insulator_thickness=2;                     /*thickness of insulating layer- VARIABLE*/
const int right_insulator_thickness=2;                    /*thickness of right insulating layer- VARIABLE*/

const int TS_num=5;                                            /*tunnel spacer thickness- VARIABLE*/
...
double W_B=1;                           /*band gap width parameter- VARIABLE*/
double W_FM=1;                              /*HG width parameter- VARIABLE*/
...
/*This function evalutes the spin current densities for the STT and IEC terms as well as carry out the STT theta integration*/
  ...
  int total_thickness=1+left_insulator_thickness+TS_num+right_insulator_thickness+FM_l_num+S_num+FM_r_num+1;                                                                                                                                            /*total thickness ie. all layers plus two- VARIABLE*/

  int cleavage=left_insulator_thickness+TS_num+right_insulator_thickness+FM_l_num+(0.5*S_num);                                                                                                                                      /*location of cleavage ie. index of plane to left of cleavage- VARIABLE*/
  ...
  complex<double> u_r_ins_1(-0.22371,0),u_r_ins_2(0,0),u_r_ins_3(0,0),u_r_ins_4(0.401852,0);                                                                                                     /*on-site of right insulator- VARIABLE*/
  ...
  complex<double> u_tunnel_spacer_1(-0.600624+(0.5*voltage),0),u_tunnel_spacer_2(0,0),u_tunnel_spacer_3(0,0),u_tunnel_spacer_4(0.513965+(0.5*voltage),0);                 /*on-site for spacer in tunnel system- VARIABLE*/
  ...
  complex<double> t_r_ins_1(0.0497084,0),t_r_ins_2(0.0101115*W_B,0),t_r_ins_3(0.0101115*W_B,0),t_r_ins_4(-0.0866541,0);                                                                             /*on-site of right insulator- VARIABLE*/
  ...
  complex<double> t_tunnel_spacer_1(0.0620793,0),t_tunnel_spacer_2(0.0244402,0),t_tunnel_spacer_3(0.0244402,0),t_tunnel_spacer_4(-0.160936,0);
  ...
  MatrixXcd u_r_ins(N,N);
  u_r_ins<<u_r_ins_1,u_r_ins_2,u_r_ins_3,u_r_ins_4;                                      /*VARIABLE*/
  ...
  MatrixXcd u_tunnel_spacer(N,N);                                          /*VARIABLE*/
  u_tunnel_spacer<<u_tunnel_spacer_1,u_tunnel_spacer_2,u_tunnel_spacer_3,u_tunnel_spacer_4;
  ...
  MatrixXcd t_r_ins(N,N);
  t_r_ins<<t_r_ins_1,t_r_ins_2,t_r_ins_3,t_r_ins_4;                                      /*VARIABLE*/
  ...
  MatrixXcd t_tunnel_spacer(N,N);                                 /*VARIABLE*/
  t_tunnel_spacer<<t_tunnel_spacer_1,t_tunnel_spacer_2,t_tunnel_spacer_3,t_tunnel_spacer_4;
  ...
  MatrixXcd v_tunnel_spacer(N,N);
  v_tunnel_spacer=(w2*I)-u_tunnel_spacer-(w1*t_tunnel_spacer);
...
/*Barrier potential drop modelled as linear*/
  ...
  if(left_insulator_thickness==1)
      ...
      for(int i=0;i<left_insulator_thickness;i++)
	{
	  ...
	  shifted_v_l_ins[i]=(w2*I)-u_l_ins-(w1*t_l_ins)-(number_1*increment_1*I)-(0.5*voltage*I);
                                   ...
                                   /*Start of shifted right insulator potential*/

  MatrixXcd shifted_v_r_ins[right_insulator_thickness];

  if(right_insulator_thickness==1)
    {
      shifted_v_r_ins[0]=(w2*I)-u_r_ins-(w1*t_r_ins)-(0.5*voltage*I);
    }
  else
    {
      complex<double> increment_2(0.5*voltage/(right_insulator_thickness-1),0);

      for(int i=0;i<right_insulator_thickness;i++)
	{
	  complex<double> number_2(i,0);
	  shifted_v_r_ins[i]=(w2*I)-u_r_ins-(w1*t_r_ins)-(number_2*increment_2*I);
	}
    }
  

                                     /*End of shifted right insulator potential*/
 ...
 /*spin up v's*/
  ...
  for(int i=left_insulator_thickness+1;i<=left_insulator_thickness+TS_num;i++)
    {
      paras_v_up[i]=v_tunnel_spacer;
    }
  for(int i=left_insulator_thickness+TS_num+1;i<=left_insulator_thickness+TS_num+right_insulator_thickness;i++)
    {
      paras_v_up[i]=shifted_v_r_ins[left_insulator_thickness+TS_num+right_insulator_thickness-i];
    }
  for(int i=left_insulator_thickness+TS_num+right_insulator_thickness+1;i<=left_insulator_thickness+TS_num+right_insulator_thickness+FM_l_num;i++)
    {
      paras_v_up[i]=v_l_FM_up;
    }
  for(int i=left_insulator_thickness+TS_num+right_insulator_thickness+FM_l_num+1;i<=left_insulator_thickness+TS_num+right_insulator_thickness+FM_l_num+S_num;i++)
    {
      paras_v_up[i]=v_spacer;
    }
  for(int i=left_insulator_thickness+TS_num+right_insulator_thickness+FM_l_num+S_num+1;i<=left_insulator_thickness+TS_num+right_insulator_thickness+FM_l_num+S_num+FM_r_num;i++)
    {
  ...
  for(int i=left_insulator_thickness+1;i<=left_insulator_thickness+TS_num;i++)
    {
      paras_v_down[i]=v_tunnel_spacer;
    }
  for(int i=left_insulator_thickness+TS_num+1;i<=left_insulator_thickness+TS_num+right_insulator_thickness;i++)
    {
      paras_v_down[i]=shifted_v_r_ins[left_insulator_thickness+TS_num+right_insulator_thickness-i];
    }
  for(int i=left_insulator_thickness+TS_num+right_insulator_thickness+1;i<=left_insulator_thickness+TS_num+right_insulator_thickness+FM_l_num;i++)
    {
  ...
  for(int i=left_insulator_thickness+1;i<=left_insulator_thickness+TS_num;i++)
    {
      paras_t_up[i]=t_tunnel_spacer;
    }
  for(int i=left_insulator_thickness+TS_num+1;i<=left_insulator_thickness+TS_num+right_insulator_thickness;i++)
    {
      paras_t_up[i]=t_r_ins;
    }
  for(int i=left_insulator_thickness+TS_num+right_insulator_thickness+1;i<=left_insulator_thickness+TS_num+right_insulator_thickness+FM_l_num;i++)
    {
  ...
  for(int i=left_insulator_thickness+1;i<=left_insulator_thickness+TS_num;i++)
    {
      paras_t_down[i]=t_tunnel_spacer;
    }
  for(int i=left_insulator_thickness+TS_num+1;i<=left_insulator_thickness+TS_num+right_insulator_thickness;i++)
    {
      paras_t_down[i]=t_r_ins;
    }
  for(int i=left_insulator_thickness+TS_num+right_insulator_thickness+1;i<=left_insulator_thickness+TS_num+right_insulator_thickness+FM_l_num;i++)
    {
